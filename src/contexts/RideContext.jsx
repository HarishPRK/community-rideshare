...
export const RideProvider = ({ children }) => {
  ...
  // Mock data for rides (Wrapped in useMemo for stable reference)
  const mockRideData = useMemo(() => [
    {
      id: "ride1",
      pickupLocation: {
  ...
  // Wrap in useCallback to stabilize its reference for other hooks
  ...
  }, [fetchUserRides, fetchAvailableRides]); // Dependencies for debouncedRefresh

  // Request to join an existing ride (used by RideDetailsPage button)
  const requestToJoinRide = useCallback(async (rideId) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const requestRide = useCallback(async (requestData) => {
    setLoading(true);
    setError(null);
  ...
  }, [mockRideData]); // Removed unnecessary debouncedRefresh dependency

  // Accept a ride request (for drivers)
  const acceptRideRequest = useCallback(async (rideId) => {
    setLoading(true);
    setError(null);
  ...
  const submitRating = useCallback(async (rideId, ratingData) => {
    setLoading(true);
    setError(null);
  ...
  const searchRides = useCallback(async (searchCriteria) => {
    setLoading(true);
    setError(null);
  ...
  }, [mockRideData]); // Added mockRideData dependency
